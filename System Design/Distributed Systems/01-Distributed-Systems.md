...
- __Distributed system__ - multiple independent nodes communicating over a network
- __Goal__ - behave like a single coherent system despite network delays, failures, and concurrency
- __Core challenges__ -
  - No global clock - hard to order events
  - Partial failures - ome nodes fail, others continue
  - Independent execution - no centralized control

## System Models

- Define assumptions about time and communication

- __Synchronous system__ -
  - Assumptions -
    - Bounded message delays
    - Bounded execution time
    - Clocks synchronized (or negligible drift)
  - Implications -
    - Time is reliable
    - Timeout means actual failure
    - Enables round-based execution - send → receive all → compute
    - Deterministic algorithms possible
  - Mostly theoretical

- __Asynchronous system__ -
  - Assumptions -
    - No bounds on delay or execution
    - No global clock
  - Implications -
    - Time is unreliable
    - Cannot distinguish slow node vs failed node
    - Timeouts are heuristics
    - Cannot rely on time for correctness

- __Partial Synchrony__ - 
  - System is async but behaves “stable enough” eventually
  - Uses -
    - timeouts
    - retries
    - SLAs
...
- __Fail-stop__ -
  ...
  - Failure is detectable by others (eg, explicit signal)
  - Simplified model - rarely perfect but good approximation

- __Crash (Crash-Stop / Crash-Recovery)__ -
  - Node halts silently
...
- __Timeouts__ -
  - Simple, heuristic-based detection - if node doesn’t respond within threshold then suspect failure
...
- Complications -
  ...
  - May require significant hardware or trade-offs in other properties (eg, performance vs consistency)
...
- Approaches -
  ...
  - Automated - Followers detect leader crash (eg, heartbeats) and elect new leader - faster but risky
...
- Suitable for applications where access and speed matter more than immediate consistency (eg, shopping carts)
...
- Timing -
  ...
  - Lazy Resolution - Maintain multiple versions - resolve later, eg, during a read
- Common Approaches -
